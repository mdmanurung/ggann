...
from ggann._expression import densify_frame, project_expression, resolve_source
...
def test_projection_limits_matrix_before_annplyr(matrix_format):
    ...
    actual = (
        projected.X.toarray()
        if sparse.issparse(projected.X)
        else np.asarray(projected.X)
    )
...
def test_backed_raw_resolution_and_aggregation_match_memory(matrix_format, tmp_path):
    ...
    expected_aggregate = aggregate_expression(
        adata, ["gene_3", "gene_9"], "group", use_raw=True
    )
    ...
    try:
        ...
        actual_aggregate = aggregate_expression(
            backed, ["gene_3", "gene_9"], "group", use_raw=True
        )
    finally:
        backed.file.close()
...
def test_projected_aggregation_matches_manual_means_and_fractions(
    matrix_format, expression_cutoff
):
    adata = _adata(matrix_format)
    ...
    actual_mean = result.pivot(
        index="group", columns="feature", values="mean_expression"
    )
...
def test_tidy_expression_deduplicates_requested_genes():
    adata = _adata("csr")
    result = tidy_expression(
        adata, ["gene_1", "gene_1", "gene_2"], "group", use_raw=False
    )
...
def test_highest_expression_sparse_and_dense_have_same_plot_data():
    ...
    assert np.allclose(
        dense_plot.data["percent"], sparse_plot.data["percent"], equal_nan=True
    )
